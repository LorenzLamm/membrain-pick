        ...
        for key in keys:
            ...
            if self.apply_on_separate_channels:
                for i in range(transform_array.shape[1]):
                    transform_array[:,i] = (transform_array[:,i] - transform_array[:,i].mean()) / transform_array[:,i].std()
            else:
                transform_array = (transform_array - transform_array.mean()) / transform_array.std()
...
class RandomGaussianSmoothing(PointCloudAugmentation):
    def __init__(self, apply_prob=0.5, smoothing_radius_range=(0.1, 0.5), smoothing_sigma_range=(0.1, 0.5), **kwargs):
        super().__init__(**kwargs)
    ...
    def _randomize(self):
        self.smoothing_radius = np.random.uniform(self.smoothing_radius_min, self.smoothing_radius_max)
        self.smoothing_sigma = np.random.uniform(self.smoothing_sigma_min, self.smoothing_sigma_max)
    ...
    def __call__(self, point_cloud_dict, keys, mb_tree, *args, **kwargs):
        ...
        if np.random.rand() < self.apply_prob:
            for key in keys:
                transform_array = self.extract_transform_array(point_cloud_dict, key)
                transform_array = apply_gaussian_filter(point_cloud_dict[key][:, :3],
                                                            transform_array,
                                                            tree=mb_tree,
...
class RandomMedianSmoothing(PointCloudAugmentation):
    ...
    def __call__(self, point_cloud_dict, keys, mb_tree, *args, **kwargs):
        ...
        if np.random.rand() < self.apply_prob:
            for key in keys:
                transform_array = self.extract_transform_array(point_cloud_dict, key)
                transform_array = apply_median_filter(point_cloud_dict[key][:, :3],
                                                        transform_array,
                                                        tree=mb_tree,
                                                        radius=self.smoothing_radius)
                point_cloud_dict = self.insert_transform_array(point_cloud_dict, key, transform_array)
...
class RandomFeatureDropout(PointCloudAugmentation):
    def __init__(self, apply_prob=0.5, dropout_prob_range=(0., 0.3), **kwargs):
        super().__init__(**kwargs)
    ...
    def _randomize(self):
        self.dropout_prob = np.random.uniform(self.dropout_prob_min, self.dropout_prob_max)
...
class RandomFeatureNoise(PointCloudAugmentation):
    def __init__(self, apply_prob=0.5, noise_std_range=(0., 0.3), **kwargs):
        super().__init__(**kwargs)
...
class RandomFeatureScale(PointCloudAugmentation):
    ...
    def __call__(self, point_cloud_dict, keys, *args, **kwargs):
        ...
        if np.random.rand() < self.apply_prob:
            for key in keys:
                transform_array = self.extract_transform_array(point_cloud_dict, key)
                transform_array = scale_point_cloud_features(transform_array, self.scale, self.preserve_range)
                point_cloud_dict = self.insert_transform_array(point_cloud_dict, key, transform_array)
    ...
    def __call__(self, point_cloud_dict, keys, *args, **kwargs):
        if np.random.rand() < self.apply_prob:
            for key in keys:
                ...
                self._randomize()
                transform_array = scale_point_cloud_stats_inversion(transform_array, self.gamma)
                self._randomize()
                transform_array = scale_point_cloud_stats_inversion(transform_array, self.gamma)
                point_cloud_dict = self.insert_transform_array(point_cloud_dict, key, transform_array)
...
class RandomErasing(PointCloudAugmentation):
    def __init__(self, apply_prob=0.5, patch_radius_range=(0.1, 0.5), num_patches_range=(1, 5), **kwargs):
        super().__init__(**kwargs)
...
class RandomBrightnessGradient(PointCloudAugmentation):
    def __init__(self, apply_prob=0.5, max_brightness_gradient_strength=1.0, brightness_gradient_scale_max=1.0, **kwargs):
        super().__init__(**kwargs)
    ...
    def _randomize(self):
        self.brightness_gradient_strength = np.random.uniform(-self.max_brightness_gradient_strength, self.max_brightness_gradient_strength)
        self.brightness_gradient_scale = np.random.uniform(0, self.brightness_gradient_scale_max)
    ...
    def __call__(self, point_cloud_dict, keys, *args, **kwargs):
        ...
        if np.random.rand() < self.apply_prob:
            for key in keys:
                transform_array = self.extract_transform_array(point_cloud_dict, key)
                transform_array = apply_gradient_to_pointcloud(point_cloud_dict[key][:, :3],
                                                                transform_array,
...
class RandomLocalBrightnessGamma(PointCloudAugmentation):
    ...
    def __call__(self, point_cloud_dict, keys, *args, **kwargs):
        if np.random.rand() < self.apply_prob:
            for key in keys:
                transform_array = self.extract_transform_array(point_cloud_dict, key)
                transform_array = apply_local_gamma_to_pointcloud(point_cloud_dict[key][:, :3],
                                                                transform_array,
...
def apply_gaussian_filter_along_features(features, radius, sigma):
    ...
    for i in range(features.shape[1]):
        ...
        i_within_radius = i_within_radius[(i_within_radius >= 0) & (i_within_radius < features.shape[1])]
...
class RandomSmoothPointFeatures(PointCloudAugmentation):
    def __init__(self, apply_prob=0.5, smoothing_range=(0, 5), smoothing_sigma_range=(0.1, 0.5), **kwargs):
        super().__init__(**kwargs)
    ...
    def __call__(self, point_cloud_dict, keys, *args, **kwargs):
        ...
        if np.random.rand() < self.apply_prob:
            for key in keys:
                transform_array = self.extract_transform_array(point_cloud_dict, key)
                transform_array = apply_gaussian_filter_along_features(transform_array, 
                                                                       self.smoothing, 
...
def test():
    ...
    from membrain_pick.optimization.plane_projection import make_2D_projection_scatter_plot, get_sample_point_cloud
    print("Hi, this is a test function for point_cloud_augmentations.py")

    # point_cloud = {
    #     "features": np.random.rand(100, 9) # 100 points, 6 features, first 3 are positions
    # }
    # mb_tree = KDTree(point_cloud["features"][:, :3])
