...
def compute_positions_along_normals(
    ...
    positions = verts[:, None, :] + normals[:, None, :] * np.arange(steps[0], steps[1])[None, :, None] * tomo_step_size
...
def convert_seg_to_evenly_spaced_mesh(
    ...
    mesh.points *= input_pixel_size # rescale to phyiscal size to compute barycentric area
